...
func TestAgentSendPersistsTurnAndProviderMetricsAcrossReopen(t *testing.T) {
	...
	if len(completer.calls) != 2 || completer.calls[0].Kind != "main" || completer.calls[1].Kind != "memory" {
		t.Fatalf("completion calls = %#v, want main then memory", completionKinds(completer.calls))
	}
	...
	if len(stats) != 2 {
		t.Fatalf("reopened stats rows = %d, want main and memory", len(stats))
	}
	stat := stats[0]
	...
	if memory := stats[1]; memory.Kind != "memory" || memory.Calls != 1 {
		t.Fatalf("reopened memory metric = (%q, %d), want (memory, 1)", memory.Kind, memory.Calls)
	}
}
